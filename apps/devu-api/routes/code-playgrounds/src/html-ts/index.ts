...
export async function* exec(input: Input, config: ExecConfig): AsyncGenerator<ExecEvent> {
  ...
  const transpiler = new Bun.Transpiler({
    ...
    tsconfig: {
      compilerOptions: config.options.tsCompilerOptions,
    },
  })
  ...
  try {
    ...
    try {
      transpiledCode = transpiler.transformSync(input.ts, 'ts')
      yield {
        type: 'result',
        data: { key: 'transpiledTs', value: transpiledCode },
        error: null,
      }
    }
    catch (error) {
...
<body>
  ...
  <script type="module">
    ...
    console.info(`[code-playground:html-ts] Execution ${config.id} complete`)
  }
  catch (error) {
    yield {
      type: 'result',
