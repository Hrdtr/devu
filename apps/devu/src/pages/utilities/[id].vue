...
const meta = ref<null | undefined | import('@api/utilities/_shared/types').Meta>(null)
onMounted(async () => {
...
async function submit() {
  if (inputRef.value) {
    ...
    const values = { input: inputValues, options: optionsValues }
...
<template>
  <div v-if="meta" class="p-4 grid grid-cols-1 @3xl:grid-cols-2 gap-4">
    <form class="@container" @submit.prevent="submit">
      <JSONSchemaForm ref="input" as="fieldset" :schema="meta.schema.input" class="[&_button[type=submit]]:hidden" />
      <Separator v-if="meta.schema.options" class="my-4" />
      <JSONSchemaForm
        v-if="meta.schema.options" ref="options" as="fieldset" :schema="meta.schema.options"
