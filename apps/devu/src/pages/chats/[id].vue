...
} = useLLMChatMessages(String(route.params.id), {
...
onMounted(async () => {
  ...
  const { data, error } = await safe(client.llmChat.retrieve({ id: String(route.params.id) }))
  if (error) {
  ...
  chat.value = data
  title.value = `Chat: ${chat.value.title || 'Untitled'}`
})

onActivated(() => {
  if (route.params.id === 'new') {
    title.value = 'Chat: New'
    return
  }
  if (chat.value) {
    title.value = `Chat: ${chat.value.title}`
  }
})
...
<template>
  <div class="w-full h-full flex flex-col relative">
    <div v-if="route.params.id === 'new'" class="w-full h-full grid place-items-center">
      ...
      <div class="w-full max-w-screen-md mx-auto p-3.5">
        <div class="w-full bg-sidebar text-sidebar-foreground rounded-xl">
          ...
          <div class="px-3 flex flex-row justify-between gap-4 pb-3">
                  ...
                  <SelectGroup>
                    <ResponsiveDialog v-slot="{ isDesktop }">
                      ...
                      <ResponsiveDialogContent class="max-h-[80vh] flex flex-col p-0">
                        ...
                        <div class="flex flex-col overflow-y-auto w-full max-w-lg mx-auto px-5 py-4">
                              ...
                              <div class="flex flex-row gap-1">
                                ...
                                >
                                  ...
                                  <ResponsiveDialogContent class="max-h-[80vh] flex flex-col p-0">
                                    ...
                                    <div class="overflow-y-auto px-5 pt-4 pb-5">
                        ...
                        <ResponsiveDialogFooter v-if="!isDesktop" class="w-full max-w-lg mx-auto px-5 pb-5">
                          <Button
                            type="button"
                            variant="secondary"
                            size="lg"
            ...
            <div class="flex flex-row gap-1">
              <Button v-if="messageState !== 'streaming'" type="submit" :disabled="messageState !== 'idle'">
                <ArrowUp /> Send
              </Button>
