...
import { useClipboardItems, useLocalStorage, useScroll } from '@vueuse/core'
import hljs from 'highlight.js'
import { ArrowUp, Check, Cog, Copy, Pencil, RotateCcw, Square, Trash, X } from 'lucide-vue-next'
...
const { profiles, deleteProfile } = useLLMChatProfiles()
const { chats, createChat } = useLLMChat()
const { messages, sendMessage, regenerateMessage, editMessage, loading, state, stopActiveMessageStream, getAvailableBranches, currentBranchId, switchBranch } = useLLMChatMessages(String(route.params.id))
...
async function scrollToBottom() {
  await nextTick()
  setTimeout(() => {
    virtualizerRef.value?.scrollToIndex(messages.value.length - 1, { align: 'start' })
  }, 0)
}
async function scrollToMessageIndex(...args: Parameters<NonNullable<typeof virtualizerRef.value>['scrollToIndex']>) {
  await nextTick()
  setTimeout(() => {
    virtualizerRef.value?.scrollToIndex(...args)
  }, 0)
}
onActivated(scrollToBottom)
watch(virtualizerRef, scrollToBottom)
watch(currentBranchId, async (branchId) => {
  if (!branchId)
    return
  const fistBranchMessageIndex = messages.value.findIndex(m => m.branch_id === branchId)
  if (
    fistBranchMessageIndex === -1
    || messages.value[fistBranchMessageIndex].branch_id === branchId // Prevent scroll if it's not newly created branch
  ) {
    return
  }
  await scrollToMessageIndex(fistBranchMessageIndex)
})
...
// Update chat title
watch(() => chats.value.find(c => c.id === route.params.id), (value) => {
  if (value) {
...
async function initializeProfile() {
  const assistantMessages = messages.value.filter(i => i.role === 'assistant')
...
watch([profiles, loading], initializeProfile)
...
watch(editMessageId, (value) => {
  editMessageContent.value = value ? (messages.value.find(m => m.id === value)?.content || '') : ''
})
...
async function submitMessage() {
  ...
  if (route.params.id === 'new') {
    ...
    messageContent.value = ''
    await router.replace(`/chats/${newChat.id}`)
  }
  else {
...
<template>
  <div class="w-full h-full flex flex-col relative">
    <div v-if="messages.length === 0" class="w-full h-full grid place-items-center">
    ...
    <ListboxRoot v-else class="flex-1 h-dvh -mt-[80px]" disabled>
      <ListboxContent
        ref="chatContainer"
        class="w-full h-full overflow-y-auto pt-[80px]" :style="{ paddingBottom: `${(formContainerRef?.offsetHeight || 0)}px` }"
      >
        <ListboxVirtualizer
          v-slot="{ option: message, virtualizer, virtualItem }" :options="messages"
          :text-content="(opt) => opt.content" :overscan="3"
        >
          <div
            :ref="(el) => {
              if (el) {
            ...
            <ListboxItem :key="message.id" :value="message.id" class="w-full">
              <div class="w-full max-w-screen-md mx-auto p-4 mb-4 space-y-2">
                ...
                <div v-else class="mb-1">
                  <div v-if="state === 'pending' && message.content.length === 0" class="flex space-x-0.5 items-center">
                  ...
                  <div
                    v-else :key="message.id" class="prose dark:prose-invert [&_pre:has(code)]:p-0 select-auto"
                    v-html="parseAssistantMessageContent(message.content)"
                  />
                </div>
                <!-- <div>{{ message.branch_id }}</div> -->

                <div class="flex flex-row -mx-2 mb-1" :class="message.role === 'user' ? 'justify-end' : ''">
                  <template v-if="message.role === 'user'">
                    <LLMChatMessageBranchSelection
                      :chat-id="String(route.params.id)" :message="message"
                      :get-available-branches="getAvailableBranches" :disabled="state === 'pending'"
                      @switch-branch="switchBranch"
                    />

                    <template v-if="virtualItem.index === messages.length - 2">
                      <Popover v-if="state !== 'pending'" :open="editMessageId === message.id">
                        <PopoverTrigger as-child>
                          <Button
                            :variant="editMessageId === message.id ? 'secondary' : 'ghost'" size="icon"
                            class="size-8 !p-0"
                        ...
                        <PopoverContent
                          side="top" align="end" :align-offset="-24"
                        ...
                        >
                          ...
                          >
                            <textarea
                              v-model="editMessageContent" class="w-full focus:outline-none px-3 pt-3" rows="4"
                              autofocus
                            />
                      ...
                      <Button v-else variant="ghost" size="icon" class="size-8 !p-0" disabled>
                        <Pencil class="size-4" />
                      </Button>
                    </template>

                    <Button
                      variant="ghost" size="icon" class="size-8 !p-0" :disabled="state === 'pending'" @click="() => {
                    ...
                    >
                      <component
                        :is="copiedMessageId === message.id ? Check : Copy" class="size-4"
                  ...
                  <template v-else>
                    ...
                    <Button
                      v-if="virtualItem.index === messages.length - 1" variant="ghost" size="icon"
                      class="size-8 !p-0" :disabled="state === 'pending'" @click="() => {
                        if (!profileSelected) {
                          toast.error('Please select a profile first')
                          return
                        }
                ...
                <p class="text-xs text-gray-500" :class="message.role === 'user' ? 'text-right' : ''">
                  {{
                    titleCase(message.role) }}<span v-if="message.role === 'assistant'" class="font-semibold">&nbsp;({{
                    message.metadata.provider }}: {{
                    message.metadata.model }})</span> -
                  {{
    ...
    <form
      ref="formContainer" class="absolute bottom-0 w-full bg-transparent" :disabled="loading || state === 'pending'"
      @submit.prevent="submitMessage"
    >
      <div class="w-full max-w-screen-md mx-auto p-3.5">
        <div class="w-full bg-sidebar text-sidebar-foreground rounded-xl">
          <textarea
            v-model="messageContent" class="w-full focus:outline-none px-3 pt-3" placeholder="Type your message here..."
          ...
          <div class="px-3 flex flex-row justify-between gap-4 pb-3">
            <div class="flex flex-row gap-1">
              <Select v-model="profileSelected" required>
                ...
                <SelectContent>
                  ...
                  <SelectGroup>
                    <ResponsiveDialog v-slot="{ isDesktop }">
                      ...
                      <ResponsiveDialogContent class="max-h-[80vh] flex flex-col">
                        ...
                        <div class="flex flex-col overflow-y-auto w-full max-w-lg mx-auto pt-4">
                          <div class="mb-4" :class="isDesktop ? 'pb-4' : ''">
                            <div
                          ...
                          <Button
                            v-if="isDesktop" type="button" variant="secondary" size="lg" class="sticky bottom-0"
                        ...
                        <ResponsiveDialogFooter v-if="!isDesktop" class="w-full max-w-lg mx-auto">
                          <Button type="button" variant="secondary" size="lg" @click="createProfileDialogOpen = true">
                            Create New Profile
                          </Button>
            ...
            <div class="flex flex-row gap-1">
              <Button v-if="state === 'idle'" type="submit" :disabled="loading">
                <ArrowUp /> Send
              </Button>
